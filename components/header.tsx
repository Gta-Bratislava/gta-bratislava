    ...
    <header className="sticky top-0 z-50 border-b border-acid/20 bg-black/90 text-white backdrop-blur-xl">
      <div className="shell flex h-[74px] items-center justify-between gap-5">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <div className="flex rounded-lg border border-white/15 bg-white/5 p-1" aria-label="Language">
        ...
        <button className="grid h-11 w-11 place-items-center rounded-xl border border-white/20 text-white md:hidden" onClick={() => setOpen(!open)} aria-expanded={open} aria-label="Menu">{open ? <X /> : <Menu />}</button>
      </div>
      {open && (
