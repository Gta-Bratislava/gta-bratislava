...
export function FinancingAdmin({ role, preview, setNotice }: { role: AdminRole; preview: boolean; setNotice: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    void db.from("financing_settings").select("*").eq("id", true).maybeSingle().then(({ data, error }) => {
      ...
      else if (data) setSettings({ ...defaultFinancingSettings, ...(data as FinancingSettings), default_interest_rate: 14, fixed_fee: 0, percent_fee: 0, localized: { ...defaultFinancingSettings.localized, ...(data as FinancingSettings).localized } });
    });
  }, [preview, setNotice]);
  ...
  async function save() {
    const normalized = { ...settings, default_interest_rate: 14, fixed_fee: 0, percent_fee: 0, allowed_terms: [...new Set(settings.allowed_terms.filter((value) => Number.isInteger(value) && value >= settings.min_term && value <= settings.max_term))].sort((a, b) => a - b) };
