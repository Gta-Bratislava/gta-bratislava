...
import { BadgeEuro, BarChart3, CalendarDays, CarFront, Gauge, Loader2, LogOut, Megaphone, Settings, ShieldCheck, Users } from "lucide-react";
...
import { FinancingAdmin } from "@/components/admin/financing-admin";
import { AdGeneratorAdmin } from "@/components/admin/ad-generator-admin";
...
type Tab = "dashboard" | "cars" | "crm" | "calendar" | "financing" | "ads" | "settings" | "analytics" | "team";
...
export function AdminPortal() {
  ...
  if (authState === "config") return <AdminMessage title={adminRu.auth.noSupabase} text={adminRu.auth.noSupabaseText} />;
  ...
  if (authState === "forbidden") return <AdminMessage title={adminRu.auth.denied} text={adminRu.auth.deniedText} logout />;
...
function AdminMessage({ title, text, logout = false }: { title: string; text: string; logout?: boolean }) {
  ...
  return <div className="grid min-h-screen place-items-center p-5"><div className="max-w-xl rounded-3xl bg-white p-8 shadow-lift"><Logo /><h1 className="mt-10 text-3xl font-black">{title}</h1><p className="mt-3 text-black/55">{text}</p>{logout && <button onClick={() => void signOut()} className="btn btn-dark mt-6"><LogOut size={17} />{adminRu.auth.signOut}</button>}</div></div>;
}
...
function AdminWorkspace({ profile, preview }: { profile: AdminProfile; preview: boolean }) {
  ...
  const [notice, setNotice] = useState<string>(preview ? adminRu.common.previewNotice : adminRu.common.loading);
  ...
  useEffect(() => {
    ...
    async function load() {
      const requests = [
        client.from("cars").select("id,slug,data,status,price,sort_order,featured,is_new,good_price,financing_calculator_enabled,created_at,updated_at").order("sort_order", { ascending: true }),
      ...
      if (!active) return;
      if (carsResult.error) { setNotice(`Автомобили: ${carsResult.error.message}. Запустите новую SQL-миграцию.`); return; }
  ...
  const tabs = useMemo(() => [
    { id: "dashboard" as const, label: adminRu.menu.dashboard, icon: Gauge, show: true },
    { id: "cars" as const, label: adminRu.menu.cars, icon: CarFront, show: can(profile.role, "cars:view") },
    { id: "crm" as const, label: adminRu.menu.crm, icon: Users, show: can(profile.role, "crm:view") },
    { id: "calendar" as const, label: adminRu.menu.calendar, icon: CalendarDays, show: can(profile.role, "calendar:view") },
    { id: "financing" as const, label: adminRu.menu.financing, icon: BadgeEuro, show: can(profile.role, "financing:view") },
    { id: "ads" as const, label: adminRu.menu.adGenerator, icon: Megaphone, show: can(profile.role, "ads:view") },
    { id: "settings" as const, label: adminRu.menu.settings, icon: Settings, show: can(profile.role, "settings:view") },
    { id: "analytics" as const, label: adminRu.menu.analytics, icon: BarChart3, show: can(profile.role, "analytics:view") },
    { id: "team" as const, label: adminRu.menu.team, icon: ShieldCheck, show: can(profile.role, "admins:manage") },
  ].filter((item) => item.show), [profile.role]);

  async function saveCar(car: Car) {
    if (!car.slug.trim() || !(car.text.ru.title || car.text.sk.title).trim()) { setNotice(adminRu.cars.required); return false; }
    const normalized = { ...car, slug: slugify(car.slug), updatedAt: new Date().toISOString() };
    if (preview) { setCars((items) => items.some((item) => item.id === normalized.id) ? items.map((item) => item.id === normalized.id ? normalized : item) : [normalized, ...items]); setNotice(adminRu.common.previewNotice); return true; }
    const db = getSupabaseBrowserClient(); if (!db) return false;
    setBusy(true);
    const record = { id: normalized.id, slug: normalized.slug, data: normalized, status: normalized.status, price: normalized.price, sort_order: normalized.sortOrder || (cars.length + 1) * 10, featured: normalized.featured, is_new: Boolean(normalized.isNew), good_price: Boolean(normalized.goodPrice), financing_calculator_enabled: normalized.financingCalculatorEnabled !== false };
    const { data, error } = await db.from("cars").upsert(record).select("id,slug,data,status,price,sort_order,featured,is_new,good_price,financing_calculator_enabled,created_at,updated_at").single();
    setBusy(false);
    if (error) { setNotice(adminErrorMessage(error)); return false; }
    const saved = normalizeCar(data);
    setCars((items) => items.some((item) => item.id === saved.id) ? items.map((item) => item.id === saved.id ? saved : item) : [saved, ...items]); setNotice(adminRu.cars.saved); return true;
  }

  async function deleteCar(car: Car) {
    if (!window.confirm(adminRu.cars.deleteConfirm.replace("{name}", car.text.ru.title || car.text.sk.title || car.slug))) return;
    ...
    if (storagePaths.length) { const { error } = await db.storage.from("cars").remove(storagePaths); if (error) return setNotice(adminErrorMessage(error)); }
    const { error } = await db.from("cars").delete().eq("id", car.id);
    if (error) return setNotice(adminErrorMessage(error));
    setCars((items) => items.filter((item) => item.id !== car.id)); setNotice(adminRu.cars.removed);
  }
  ...
  async function duplicateCar(car: Car) {
    ...
    if (preview) {
      ...
      const db = getSupabaseBrowserClient();
      if (!db) return;
      setNotice(adminRu.cars.copyingPhotos);
      for (const [index, image] of car.images.entries()) {
        ...
        if (fullResult.error || thumbResult.error) { setNotice((fullResult.error || thumbResult.error)?.message || "Не удалось скопировать фотографии."); return; }
    ...
    const copy: Car = structuredClone({ ...car, id, slug: `${car.slug}-copy-${id.slice(0, 4)}`, status: "draft", price: commercial ? car.price : 0, monthlyPrice: commercial ? car.monthlyPrice : 0, financing: commercial ? car.financing : false, featured: false, isNew: false, images, thumbnails, sortOrder: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), text: { ...car.text, ru: { ...car.text.ru, title: `${car.text.ru.title || car.text.sk.title} — ${adminRu.cars.copiedSuffix}` }, sk: { ...car.text.sk, title: `${car.text.sk.title} — kópia` } } });
    await saveCar(copy);
  }
