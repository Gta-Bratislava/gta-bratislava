...
const text = {
  sk: { eyebrow: "Financovanie", calculator: "Kalkulačka splátky", car: "Vybrané vozidlo", ownPrice: "Vlastná cena", price: "Cena vozidla", downEur: "Akontácia v €", downPercent: "Akontácia v %", term: "Doba splácania", rate: "Ročná úroková sadzba", fixedFee: "Fixný poplatok", percentFee: "Percentuálny poplatok", monthly: "Orientačná mesačná splátka", financed: "Financovaná suma", fees: "Poplatky", total: "Celkom vrátane akontácie", overpayment: "Preplatenie", months: "mesiacov", apply: "Prejsť k žiadosti", documents: "Čo si pripraviť", process: "Ako to funguje", faq: "Časté otázky", call: "Zavolať", whatsapp: "WhatsApp", errors: { invalid_price: "Zadajte platnú cenu vozidla.", below_minimum: "Financovaná suma je nižšia ako povolené minimum.", above_maximum: "Financovaná suma presahuje povolené maximum.", invalid_term: "Vyberte povolenú dobu splácania.", down_payment_too_low: "Akontácia je nižšia ako nastavené minimum." } },
  ru: { eyebrow: "Финансирование", calculator: "Калькулятор платежа", car: "Выбранный автомобиль", ownPrice: "Своя стоимость", price: "Стоимость автомобиля", downEur: "Первоначальный взнос в €", downPercent: "Первоначальный взнос в %", term: "Срок финансирования", rate: "Годовая процентная ставка", fixedFee: "Фиксированная комиссия", percentFee: "Процентная комиссия", monthly: "Ориентировочный платёж в месяц", financed: "Сумма финансирования", fees: "Комиссии", total: "Всего с первоначальным взносом", overpayment: "Переплата", months: "месяцев", apply: "Перейти к заявке", documents: "Что подготовить", process: "Как всё проходит", faq: "Частые вопросы", call: "Позвонить", whatsapp: "WhatsApp", errors: { invalid_price: "Укажите корректную стоимость автомобиля.", below_minimum: "Сумма финансирования ниже установленного минимума.", above_maximum: "Сумма финансирования превышает установленный максимум.", invalid_term: "Выберите доступный срок финансирования.", down_payment_too_low: "Первоначальный взнос ниже установленного минимума." } },
  ua: { eyebrow: "Фінансування", calculator: "Калькулятор платежу", car: "Обраний автомобіль", ownPrice: "Власна вартість", price: "Вартість автомобіля", downEur: "Перший внесок у €", downPercent: "Перший внесок у %", term: "Строк фінансування", rate: "Річна відсоткова ставка", fixedFee: "Фіксована комісія", percentFee: "Відсоткова комісія", monthly: "Орієнтовний платіж на місяць", financed: "Сума фінансування", fees: "Комісії", total: "Усього з першим внеском", overpayment: "Переплата", months: "місяців", apply: "Перейти до заявки", documents: "Що підготувати", process: "Як усе відбувається", faq: "Часті запитання", call: "Зателефонувати", whatsapp: "WhatsApp", errors: { invalid_price: "Укажіть коректну вартість автомобіля.", below_minimum: "Сума фінансування нижча за встановлений мінімум.", above_maximum: "Сума фінансування перевищує встановлений максимум.", invalid_term: "Оберіть доступний строк фінансування.", down_payment_too_low: "Перший внесок нижчий за встановлений мінімум." } },
  en: { eyebrow: "Financing", calculator: "Payment calculator", car: "Selected vehicle", ownPrice: "Custom price", price: "Vehicle price", downEur: "Down payment in €", downPercent: "Down payment in %", term: "Financing term", rate: "Annual interest rate", fixedFee: "Fixed fee", percentFee: "Percentage fee", monthly: "Estimated monthly payment", financed: "Financed amount", fees: "Fees", total: "Total including down payment", overpayment: "Overpayment", months: "months", apply: "Continue to application", documents: "What to prepare", process: "How it works", faq: "Frequently asked questions", call: "Call", whatsapp: "WhatsApp", errors: { invalid_price: "Enter a valid vehicle price.", below_minimum: "The financed amount is below the configured minimum.", above_maximum: "The financed amount exceeds the configured maximum.", invalid_term: "Choose an available financing term.", down_payment_too_low: "The down payment is below the configured minimum." } },
} as const;
...
export function FinancingPageClient({ locale }: { locale: Locale }) {
  ...
  const [rate, setRate] = useState(defaultFinancingSettings.default_interest_rate);
  const [fixedFee, setFixedFee] = useState(defaultFinancingSettings.fixed_fee);
  const [percentFee, setPercentFee] = useState(defaultFinancingSettings.percent_fee);
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([loadFinancingSettings(), loadPublicCars()]).then(([settingsResult, carsResult]) => {
      ...
      const requiredDown = Math.min(targetPrice, Math.max(effectiveSettings.min_down_payment_eur, targetPrice * effectiveSettings.min_down_payment_percent / 100));
      setDownEur(requiredDown);
      setDownPercent(targetPrice > 0 ? Math.round(requiredDown / targetPrice * 10_000) / 100 : 0);
      if (settingsResult.status === "fulfilled" && settingsResult.value) {
        setSettings(effectiveSettings);
        setRate(settingsResult.value.default_interest_rate);
        setFixedFee(settingsResult.value.fixed_fee);
        setPercentFee(settingsResult.value.percent_fee);
        setTerm(settingsResult.value.allowed_terms.includes(60) ? 60 : (settingsResult.value.allowed_terms[0] || settingsResult.value.min_term || 12));
      }
      setCars(loadedCars);
    });
    return () => { active = false; };
  ...
  const input = useMemo(() => ({ carPrice: price, downPaymentEur: downEur, termMonths: term, annualInterestRate: rate, fixedFee, percentFee }), [price, downEur, term, rate, fixedFee, percentFee]);
    ...
    <section className="section-pad"><div className="shell">
      {settings.enabled ? <div className="grid gap-7 xl:grid-cols-[1.05fr_.95fr]">
        <article className="neon-panel rounded-[24px] p-5 sm:p-8"><div className="flex items-center gap-3"><Calculator className="text-acid"/><h2 className="text-2xl font-black">{t.calculator}</h2></div><div className="mt-7 grid gap-4 sm:grid-cols-2">
          ...
          <label className="label">{t.rate}, %<input className="field" type="number" min="0" max="100" step="0.01" value={rate} onChange={(event) => setRate(Math.max(0, Number(event.target.value) || 0))}/></label>
          <label className="label">{t.fixedFee}, €<input className="field" type="number" min="0" value={fixedFee} onChange={(event) => setFixedFee(Math.max(0, Number(event.target.value) || 0))}/></label>
          <label className="label sm:col-span-2">{t.percentFee}, %<input className="field" type="number" min="0" max="100" step="0.01" value={percentFee} onChange={(event) => setPercentFee(Math.max(0, Number(event.target.value) || 0))}/></label>
        </div>
        {errors.length > 0 && <div className="mt-5 rounded-xl border border-amber-400/35 bg-amber-400/10 p-4 text-sm text-amber-100" role="alert">{errors.map((error) => <p key={error}>• {t.errors[error as keyof typeof t.errors]}</p>)}</div>}
        </article>
        <aside className="rounded-[24px] border border-acid/35 bg-acid/10 p-5 sm:p-8"><p className="text-sm font-black uppercase tracking-widest text-acid">{t.monthly}</p><p className="mt-3 text-5xl font-black tracking-tight text-white">{euro.format(calculation.monthlyPayment)}</p><div className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">{[[t.price, euro.format(calculation.carPrice)],[t.downEur, euro.format(calculation.downPaymentEur)],[t.financed, euro.format(calculation.financedAmount)],[t.fees, euro.format(calculation.fees)],[t.total, euro.format(calculation.totalPayments)],[t.overpayment, euro.format(calculation.overpayment)],[t.term, `${calculation.termMonths} ${t.months}`]].map(([label, value]) => <div key={String(label)} className="flex items-center justify-between gap-4 border-t border-white/10 pt-3"><span className="text-sm text-white/50">{label}</span><strong className="text-right">{value}</strong></div>)}</div><p className="mt-7 text-xs leading-relaxed text-white/50">{content.warning}</p><a href="#application" className="btn btn-primary mt-6 w-full">{t.apply}</a></aside>
