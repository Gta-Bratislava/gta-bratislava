...
const labels = {
  sk: { title: "Orientačné financovanie", from: "odhadovaná splátka od", term: "na {months} mesiacov", button: "Vypočítať financovanie", apply: "Odoslať žiadosť" },
  ru: { title: "Предварительный расчёт", from: "ориентировочный платёж от", term: "на {months} месяцев", button: "Рассчитать финансирование", apply: "Подать заявку" },
  ua: { title: "Попередній розрахунок", from: "орієнтовний платіж від", term: "на {months} місяців", button: "Розрахувати фінансування", apply: "Подати заявку" },
  en: { title: "Initial estimate", from: "estimated payment from", term: "over {months} months", button: "Calculate financing", apply: "Apply" },
} as const;
...
export function CarFinancingCard({ car, locale }: { car: Car; locale: Locale }) {
  ...
  useEffect(() => { void loadFinancingSettings().then((value) => { if (value) setSettings({ ...defaultFinancingSettings, ...value, localized: { ...defaultFinancingSettings.localized, ...value.localized } }); }).catch(() => undefined); }, []);
  const term = settings.allowed_terms.includes(84) ? 84 : settings.allowed_terms[0] || 84;
  const down = Math.max(0, car.price - settings.max_amount, settings.min_down_payment_eur, car.price * settings.min_down_payment_percent / 100);
  const calculation = useMemo(() => calculateFinancing({ carPrice: car.price, downPaymentEur: down, termMonths: term }), [car.price, down, term]);
